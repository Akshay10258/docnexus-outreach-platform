...
function fillPreview(template: string) {
  return template
    .replace(/{{doctor_name}}/g, PREVIEW_PHYSICIAN.doctor_name)
    .replace(/{{first_name}}/g, PREVIEW_PHYSICIAN.first_name)
    .replace(/{{specialty}}/g, PREVIEW_PHYSICIAN.specialty)
    .replace(/{{affiliation}}/g, PREVIEW_PHYSICIAN.affiliation)
    .replace(/{{city}}/g, PREVIEW_PHYSICIAN.city);
}
...
export default function PreviewPanel({ steps, previewStep, onPreviewStepChange }: Props) {
      ...
      <div style={{ padding: "16px 20px", borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#E1F5EE",
            ...
            fontSize: 12, fontWeight: 500 }}>SJ</div>
          <div>
